...
let cachedCurrent: AppUser | null = null;
...
function readCurrent(): AppUser | null {
  if (typeof window === "undefined") return null;
  try { return JSON.parse(localStorage.getItem(CURRENT_KEY) || "null"); } catch { return null; }
}

function getCurrentSnapshot(): AppUser | null {
  if (typeof window === "undefined") return null;
  const raw = localStorage.getItem(CURRENT_KEY);
  if (raw === cachedRaw) return cachedCurrent;
  cachedRaw = raw;
  try { cachedCurrent = raw ? JSON.parse(raw) : null; } catch { cachedCurrent = null; }
...
export const auth = {
  isAuthed(): boolean { return !!readCurrent(); },
  current(): AppUser | null { return readCurrent(); },
  initials(): string { return initials(readCurrent()?.name ?? "User"); },
  ...
  register(name: string, email: string, password: string): { ok: boolean; error?: string } {
    ...
    const user: StoredUser = { name: name.trim(), email: e, password, role: "Operations Manager" };
    users.push(user);
    writeUsers(users);
  ...
  updateProfile(patch: Partial<Pick<AppUser, "name" | "role" | "avatar">>) {
